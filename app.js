...
function renderVisibleTransactions() {
  ...
  if (visible.length === 0) {
    ...
    DOM.transactionsList.innerHTML = visible.map(t => {
      ...
      const deleteBtn = t.rowNum
        ? `<button type="button" class="tr-delete" data-row="${t.rowNum}" title="Удалить">×</button>`
        : '';
      return `
...
function initApp() {
  ...
  // Делегированное удаление транзакций
  DOM.transactionsList.addEventListener('click', e => {
    const btn = e.target.closest('.tr-delete');
    if (!btn) return;
